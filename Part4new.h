...
template<typename T>
constexpr bool isComplex(T const &) { return __isComplex<T>::value; }
...
public:
    ...
    explicit AugMatrix(size_t row, size_t col) : row(row), col(col), data(new T[row * col]) {
...
//TEMPLATE_T
//std::complex<T> vectorDotProduct(const std::complex<T> * src1, const std::complex<T> * src2, const size_t size) {
//    std::complex<T> ret = 0;
//    for(size_t i = 0; i < size; i++) {
//        ret += src1[i] * src2[i];
//    }
...
TEMPLATE_T
SMatrix<T> matrixRowRearrange(const SMatrix<T> &src, const std::unique_ptr<int[]> &index) {
    SMatrix<T> ret(src.size);
    for (size_t i = 0; i < src.size; i++) {
...
TEMPLATE_T
SMatrix<T> matrixEigVector(const SMatrix<T> &src, const SMatrix<T> &eigValue, int maxIteration) {
    const size_t size = src.size;
...
//    using namespace std;
//    cout<<"eigValueCopied"<<endl;
//    eigValueCopied.print();
//    cout<<"src"<<endl;
//    src.print();
//    cout<<"eigValue11111"<<endl;
//    matrixSubtract(src, eigValueCopied).print();
